...
import com.alibaba.cobar.parser.util.SmallSet;
...
public final class PartitionKeyVisitor implements SQLASTVisitor {
    ...
    //add by zhangyong 2015-06-02 用于添加数据源配置
    private DataSourceConfig dataSourceConfig;

    public PartitionKeyVisitor(DataSourceConfig dataSourceConfig) {
    ...
    private boolean isRuledColumn(String tableNameUp, String columnNameUp) {
        ...
        if (dataSourceConfig != null) {
            if (dataSourceConfig.getKeyname() != null) {
                if (dataSourceConfig.getKeyname().toUpperCase().equals(columnNameUp)) {
                    return true;
                }
            }
        }
